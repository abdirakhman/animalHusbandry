    ...
    >
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate(
            data.first.go,
            data.first.params ? data.first.params : {}
          )
        }
      >
        <Text style={styles.text}>{data.first.title}</Text>
      </TouchableOpacity>
      {data.second && (
        <TouchableOpacity
          style={styles.btn}
          onPress={() =>
            navigation.navigate(
              data.second.go,
              data.second.params ? data.second.params : {}
            )
          }
        >
          <Text style={styles.text}>{data.second.title}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
export default class Profile extends React.Component {
  ...
  async componentDidMount() {
    ...
    const obj = await FileSystem.getInfoAsync(
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Item data={item.data} navigation={this.props.navigation} />
          )}
