...
function Item({ title, go, navigation }) {
    ...
    <TouchableOpacity
      style={styles.item}
      onPress={() => navigation.navigate('RequestByType', { id : go.toString() })}
    >
...
export default class Types extends React.Component {
  ...
  async componentDidMount() {
    ...
    db.transaction(tx => {
      tx.executeSql("SELECT * FROM 'types'", [], 
      (tx, res) => {
    ...
    return (
      <View style={{ flex: 1, paddingTop: 20 }}>
        <View style={styles.header}>
            ...
            />
            <TouchableOpacity style={styles.btn} onPress={() => {this.props.navigation.navigate('AddType');}}>
