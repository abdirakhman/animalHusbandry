...
export default class InsertThing extends React.Component {
        ...
        <View style={styles.container}>
          ...
          >
            {this.state.type.name ? (
              <Text style={styles.chooseText}>{this.state.type.name}</Text>
            ) : (
  ...
  _handleinsert = async () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT INTO 'animals' (id, name, gender, active, type_name, type_id, date, vaccine_history, disease_history, ancestors) VALUES (null, ?, ?, 'Alive', ?, ?, ?, ?, ?, '')",
        [
          this.state.name,
          this.state.gender,
          this.state.type.name,
